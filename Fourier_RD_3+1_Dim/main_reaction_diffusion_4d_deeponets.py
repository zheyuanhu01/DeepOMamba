...
def get_args():
    ...
    parser.add_argument('--data2cuda', type=int, default=0)
...
def Grid2Fourier(X, grid_x):
    # X shape: [N, grid_x, grid_x, grid_x, grid_t, 1]
    X = X.permute(0, 4, 1, 2, 3, 5).squeeze() # N, grid_t, grid_x, grid_x, grid_x
    X = torch.fft.rfftn(X, dim=[-3, -2, -1]) / grid_x ** 3 # N, grid_t, grid_x, grid_x, grid_x // 2 + 1
...
def Fourier2Grid(X, grid_x):
    ...
    X[:, :, 0] = X_real[:, :, 0]
    X = X * grid_x ** 3
    X = X.reshape(N, grid_t, grid_x, grid_x, grid_x // 2 + 1)
    X = torch.fft.irfftn(X, s=[grid_x] * 3, dim=[-3, -2, -1])
    return X
...
Y_train = Y_train.squeeze().permute(0, 4, 1, 2, 3)
Y_test = Y_test.squeeze().permute(0, 4, 1, 2, 3)

print(X_train.shape, y_train.shape, X_test.shape, y_test.shape)
print(Y_train.shape, Y_test.shape)
io_dim = 2 * (args.grid_x // 2 + 1) * args.grid_x ** 2
...
if args.model == "LSTM":
    ...
    model = GalerkinTransformer(dim_in=io_dim, dim_out=io_dim, dim_hid=256, depth=1, heads=4, dim_head=256, attn_type='standard', mlp_dim=256).to(device)
elif args.model == "GT":
    model = GalerkinTransformer(dim_in=io_dim, dim_out=io_dim, dim_hid=256, depth=1, heads=4, dim_head=256, attn_type='galerkin', mlp_dim=256).to(device)
elif args.model == "FT":
    model = GalerkinTransformer(dim_in=io_dim, dim_out=io_dim, dim_hid=256, depth=1, heads=4, dim_head=256, attn_type='fourier', mlp_dim=256).to(device)
elif args.model == "T":
    ...
    model = GNOT(dim_in=io_dim, dim_out=io_dim, dim_hid=256, depth=1, heads=4, dim_head=256, n_experts=2).to(device)
